...
def handle_check(
    args: argparse.Namespace, github_user: str, github_token: Optional[str]
) -> int:
    """Handles the main logic for checking commits and PRs."""
    logger.info(f"Executing DUCK check for {github_user}")
    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    logger.info(
        f"Checking for commits and PRs made by '{github_user}' on {today_str} (UTC)."
    )
    ...
    try:
        commits_found_today = find_todays_commits(
            github_user, github_token, max_event_pages=max_event_pages
        )
        prs_found_today = find_todays_prs(
            github_user, github_token, max_pr_pages=max_pr_pages
        )

        if commits_found_today or prs_found_today:
            logger.info(
                f"QUACK! Activity found for '{github_user}' today ({today_str} UTC)."
            )
            return EXIT_CODE_SUCCESS
        else:
            logger.warning(
                f"No commits or PRs found for '{github_user}' today. "
                f"Don't be a DUCK! Time to make some contributions. (Exit code {EXIT_CODE_NO_ACTIVITY})"
            )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="DUCK (Did U Commit mr.Kim?) - Checks for daily GitHub activity."
    )
    parser.add_argument(
        "--user", type=str, help="GitHub username to check."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-event-pages",
        type=int,
        help="Maximum number of commit event pages to fetch."
    )
    parser.add_argument(
        "--max-pr-pages",
        type=int,
        help="Maximum number of pull request pages to fetch."
    )
    parser.add_argument(
    ...
    args.max_event_pages = (
        ...
        else int(env_max_event_pages) if env_max_event_pages is not None
        else github_config.get("max_event_pages", DEFAULT_MAX_EVENT_PAGES)
    )
    args.max_pr_pages = (
        args.max_pr_pages
        if args.max_pr_pages is not None
        else int(env_max_pr_pages) if env_max_pr_pages is not None
        else github_config.get("max_pr_pages", DEFAULT_MAX_PR_PAGES)
    )

    if not github_user:
        logger.error(
            "GitHub username required. Please provide it via --user argument, "
            "GITHUB_USERNAME environment variable, or in config.toml."
        )
